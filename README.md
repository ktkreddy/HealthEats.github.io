...
This repo includes a **FastAPI** app exported from **root `app.py`** (`app` instance) plus **`pyproject.toml`** so Vercel detects Python alongside `package.json`. Do **not** put the FastAPI app under `api/*.py`: each file there is a **separate** function and only matches one URL (e.g. `/api` only), so **`/api/predict` returns 404**. Root `requirements.txt` and `public/` static UI. Streamlit is not run on Vercel; use the web UI at `/` or call `POST /api/predict/`. Dependencies pin **Starlette 0.41.x** (avoid Starlette 1.x on Vercel). Local API: `uvicorn app:app --host 127.0.0.1 --port 8080 --reload`. If you still see **404** in the browser: confirm the latest deployment is live. You do **not** need extra Framework or Output settings unless you changed them before—defaults usually work. If anything was set: **Root Directory** should be `.` (repo root), and **Output Directory** should be **empty** (not `public` only, or static files would deploy without Python). This repo sets **`installCommand`** in `vercel.json` so `pip install -r requirements.txt` always runs. Check **Deployments → [latest] → Build Logs** for `pip install` and **Functions** (or **Runtime Logs**) for Python.
